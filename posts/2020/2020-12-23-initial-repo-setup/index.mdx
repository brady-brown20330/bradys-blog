...
In this post, I'm going to go over how to create your very own reusable project base. This code is designed to be used to build out a React app, but with a few modifications other frameworks could be substituted.

We will start with a blank repository, and work our way up to a 'Hello World' app using React, Node, Express, Webpack and other relevant technologies.

## Some reasons to create your next project from scratch include:

  - You will have complete control over what frameworks and extensions are included in your project.

  - You will gain an understanding of the building blocks of Full-stack Javascript projects, things that perplex even experienced programmers such as Webpack, Babel and Package.json files.

  - By following this tutorial you'll have built out something that's all your own, and something that you can modify and use for years to come.
...
I am going to name this repo react-app-starter, and add a README.MD.

Once you've created your repo, clone it down to your machine, the command should look something like this:

`git clone https://github.com/brady-brown20330/react-app-starter.git`

Once that command runs, you should be able to open it up in your code editor of choice (I'll be using VS Code).

Now that we have our repository opened up, were ready to build out the skeleton that will support our project: The file structure. We will be doing this strictly from the command line, using the commands `mkdir` and `touch`.

Well start from the root directory, and work our way into the folders as we go.

Were goig to make four folders in our root directory:

- dist: This holds our main.js file, which has to do with our webpack.config.
...
- src: This will hold our app and all of the components that go along with it, as well as the index.js file (well get into what that is in a bit).

To create these first folders, make sure you are in the root directory of your project, and run this command:

`mkdir -p dist public server src/components`

Using the -p flag will allow the terminal to create parent directories if they do'nt yet exist.
...
```
The last file we have to deal with at the root level is the package.json. Since we've already run `npm init`, all we're going to do now is install some dependencies that are common to Full-stack React applications. These will be broken own into two categories; as defined by the npmjs docs:
...
Starting with the public folder, lets start adding some muscke onto our scrawny project.
